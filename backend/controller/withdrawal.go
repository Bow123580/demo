...
func GetWithdrwal(c *gin.Context) {
	var withdrawals entity.Withdrawal
	id := c.Param("id")
	if err := entity.DB().Preload("Student").Preload("Cousre").Preload("Teacher").Preload("Semester").Raw("SELECT * FROM withdrawals WHERE id = ?", id).Find(&withdrawals).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
func ListWithdrawals(c *gin.Context) {
	var withdrawals []entity.Withdrawal
	if err := entity.DB().Preload("Student").Preload("Cousre").Preload("Teacher").Preload("Semester").Raw("SELECT * FROM withdrawals ").Find(&withdrawals).Error; err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
